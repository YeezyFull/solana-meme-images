...
def load_local_image_b64(filename: str) -> Optional[str]:
    """
    Reads a local image (png/jpg/webp) next to app.py and returns a data URI.
    Cached for speed.
    """
    try:
...
def normalize_url(url: Optional[str]) -> Optional[str]:
    """
    Fix common provider URL issues:
    - Helius Cloudflare wrapper: https://cdn.helius-rpc.com/cdn-cgi/image//https://ipfs.io/ipfs/...
      -> keep only the last https://...
    - ipfs://CID/... -> https://ipfs.io/ipfs/CID/...
    - debot.ai / rs.debot.ai OSS processing params often break / 404 -> strip querystring
    - Apply hardcoded local replacements first
    """
    if not url:
        return None
    u = str(url).strip()
    ...
    # Apply special-case replacements early
    ...
    # re-apply replacements after stripping
    u = apply_replacements(u) or u
    return u
...
for r in range(0, len(items), COLS):
    ...
    for j in range(COLS):
        ...
        if not img:
            if ca in helius_map:
                img = normalize_url(nonempty_str(helius_map[ca].get("image")))
                reason = helius_map[ca].get("reason") or reason
            if not img:
                reason = "No image URL in list"
        else:
            # local replacements show fine
